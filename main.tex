...
\begin{frame}{Aggregate Computing and Supervised Learning}
  \begin{cardTiny}
    \begin{itemize}
      \item Ground truth generation is done with simulations
      \item A network is trained with a global view and then
      \item Each agent have the same network
    \end{itemize}
  \end{cardTiny}
  \begin{multicols}{3}
    \centering
    \cardImg{example-image-a}{0.18\textwidth}
...
\begin{frame}{Aggregate Computing and Evolutionary computing}
  \begin{cardTiny}
    \begin{itemize}
      \item A population of Network is shared within the entire system
      \item During the simulation, a fitness function is evaluated
      \item Already used in Swarm robotics and automatic design
    \end{itemize}
  \end{cardTiny}
  \begin{multicols}{3}
    \centering
...
\begin{frame}{Aggregate Computing and Reinforcement Learning: Hop count}
  \begin{multicols}{2}
    \begin{card}[Independent Q-Learning]
      \begin{itemize}
        \item <1-> {\color{accent} Actions:} Increase / NoIncrease
        \item <2->{\color{accent} State:} Temporal Windows of Speed
        \item <3->{\color{accent} Reward:} 0 / -1
      \end{itemize}
    \end{card}
    \only<1>{
